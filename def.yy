...
wyrprz	:	INT ID '=' wyr			{fprintf(file, "%s =", $1); argstack.push(Element(ID, $1)); insert_symbol($1, INT_TYPE, 0);make_op('=', "sw");}
		|	INT ID '=' wyrwpr		{fprintf(file, "%s =", $1); argstack.push(Element(ID, $1)); insert_symbol($1, INT_TYPE, 0);make_op('=', "sw");}
		|	FLOAT ID '=' wyr		{fprintf(file, "%s =", $1); argstack.push(Element(ID, $1)); insert_symbol($1, FLOAT_TYPE, 0);make_op('=', "sw");}
		|	FLOAT ID '=' wyrwpr		{fprintf(file, "%s =", $1); argstack.push(Element(ID, $1)); insert_symbol($1, FLOAT_TYPE, 0);make_op('=', "sw");}
		;
wyrlog	: 	wyr EQ wyr				{;}
		|	wyr NE wyr				{;}
		| 	wyr LT wyr				{;}
